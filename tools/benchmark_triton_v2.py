...
def parse_args():
    ...
    parser.add_argument(
        "--profile",
        action="store_true",
        help=("NCU profiling mode: " "5 warmup launches + 1 measured launch"),
    )
...
def print_header():
    ...
    num_programs = BATCH_SIZE * NUM_KV_HEADS * NUM_KV_SPLITS
...
def run_benchmark(
    ...
    # --------------------------------------------------------
    finite = bool(torch.isfinite(mid_o).all().item())
    nan_count = int(torch.isnan(mid_o).sum().item())
    inf_count = int(torch.isinf(mid_o).sum().item())
    if not finite:
        raise RuntimeError(f"Stage1 output invalid: " f"NaN={nan_count}, Inf={inf_count}")
    # --------------------------------------------------------
    # Warmup
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    score_evals = BATCH_SIZE * NUM_Q_HEADS * CONTEXT_LEN
    score_evals_per_s = score_evals / seconds
    # --------------------------------------------------------
    # Decode query throughput.
    #
    # One Stage1 launch processes B decode query tokens.
    # --------------------------------------------------------
    decode_tokens_per_s = BATCH_SIZE / seconds
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    logical_kv_bytes = BATCH_SIZE * NUM_KV_HEADS * CONTEXT_LEN * SLOT_SIZE_ALIGNED
    logical_kv_gb = logical_kv_bytes / 1.0e9
    logical_kv_gib = logical_kv_bytes / (1024.0**3)
    logical_kv_bw = logical_kv_bytes / seconds / 1.0e9
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    mid_o_bytes = mid_o.numel() * mid_o.element_size()
    mid_o_mib = mid_o_bytes / (1024.0**2)
    # --------------------------------------------------------
    # Useful structural values.
    # --------------------------------------------------------
    split_tokens = CONTEXT_LEN // NUM_KV_SPLITS
    tiles_per_program = split_tokens // 16
    # ========================================================
    # Print results
    # ========================================================
    print()
    ...
    print(f"Total timed GPU time  : {total_ms:.6f} ms")
    print(f"Average Stage1 time   : {avg_ms:.6f} ms")
    print(f"Average Stage1 time   : {avg_us:.3f} us")
    print()
    print(f"Decode tokens/s       : " f"{decode_tokens_per_s:,.2f}")
    print(f"Score evaluations/s   : " f"{score_evals_per_s / 1.0e9:.3f} G")
    print()
    print(f"Logical KV payload    : " f"{logical_kv_bytes:,} bytes")
    print(f"Logical KV payload    : " f"{logical_kv_gb:.3f} GB")
    print(f"Logical KV payload    : " f"{logical_kv_gib:.3f} GiB")
    print(f"Logical KV BW         : " f"{logical_kv_bw:.3f} GB/s")
    print()
    print(f"Stage1 mid_o write    : " f"{mid_o_mib:.2f} MiB")
    print()
    print(f"Tokens / split        : " f"{split_tokens}")
    print(f"KV tiles / program    : " f"{tiles_per_program}")
    print()
    ...
    print()
    print("NOTE: Logical KV BW is not DRAM bandwidth.")
    print("      Actual DRAM/L2/L1 traffic will be measured with Nsight Compute.")
