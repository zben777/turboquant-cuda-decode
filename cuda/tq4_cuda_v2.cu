...
constexpr int META_REGION_OFFSET =
...
__device__ __forceinline__ float load_meta(
    ...
    const int index =
        (kv_head * NUM_SOA_FIELDS + field) * BLOCK_SIZE
        + token_offset;
    const __half* ptr = reinterpret_cast<const __half*>(
        cache + block_base + META_REGION_OFFSET + index * 2
    );
...
#pragma unroll
    for (int j = 0; j < 4; ++j) {
        ...
        q[j] = q_rot[(static_cast<int64_t>(b) * Q_HEADS + qh)
                     * HEAD_DIM + d];
    }
    ...
    for (int token = split_start; token < split_end; ++token) {
        ...
        if (lane == 0) {
            block_num = block_table[
                static_cast<int64_t>(b) * blocks_per_seq + page_idx
            ];
        }
        block_num = __shfl_sync(FULL_MASK, block_num, 0);

        const int64_t block_base =
            static_cast<int64_t>(block_num) * cache_block_stride;
...
#pragma unroll
        for (int j = 0; j < 4; ++j) {
            ...
            const float centroid = __shfl_sync(
                FULL_MASK,
                centroid_lane,
                index
            );
        ...
        if (lane == 0) {
            ...
            alpha = running_l == 0.0f
                ? 0.0f
                : exp2f(running_m - new_m);
...
#pragma unroll
    ...
    const int64_t head_stride =
        static_cast<int64_t>(NUM_SPLITS) * (HEAD_DIM + 1);
    const int64_t out_base =
        (static_cast<int64_t>(b) * Q_HEADS + qh) * head_stride
...
torch::Tensor tq4_cuda_v2_cuda(
    ...
    TORCH_CHECK(kv_cache.size(1) == BLOCK_SIZE && kv_cache.size(2) == KV_HEADS && kv_cache.size(3) == SLOT_SIZE, "requires cache [blocks,16,8,134]");

    const int B = q_rot.size(0);
    TORCH_CHECK(mid_o.sizes() == torch::IntArrayRef({B, Q_HEADS, NUM_SPLITS, HEAD_DIM + 1}), "invalid mid_o shape");
