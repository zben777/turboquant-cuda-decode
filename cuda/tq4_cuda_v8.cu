...
__global__ __launch_bounds__(THREADS) void tq4_cuda_v8_kernel(
    ...
    __shared__ __align__(32) float qk_s[TILE][MMA_N];
...
#pragma unroll
            ...
            qk_s[group][col] = qk[0];
            qk_s[group][col + 1] = qk[1];
            qk_s[group + 8][col] = qk[2];
            qk_s[group + 8][col + 1] = qk[3];
        }
        __syncthreads();

        float score = lane < TILE ? qk_s[lane][warp] * ATTN_SCALE * RCP_LN2 : -CUDART_INF_F;
...
torch::Tensor tq4_cuda_v8_cuda(torch::Tensor q_rot, torch::Tensor kv_cache,
    ...
    tq4_cuda_v8_kernel<<<dim3(B, KV_HEADS, NUM_SPLITS), THREADS, 0, stream>>>(
