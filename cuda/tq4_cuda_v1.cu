...
constexpr int DATA_REGION_BYTES =
    BLOCK_SIZE * KV_HEADS * DATA_BYTES_PER_SLOT;
// 16 * 8 * 128 = 16384 B
...
__device__ __forceinline__
float load_half_from_bytes(
    const uint8_t* ptr
) {
    const __half* hp =
        reinterpret_cast<const __half*>(ptr);
...
const uint8_t* metadata_ptr(
    ...
    return kv_cache
        + block_base
        + META_REGION_OFFSET
        + static_cast<int64_t>(meta_index) * 2;
}
...
void tq4_cuda_v1_kernel(
    ...
    const float q0 = q_rot[
        q_base0 + 0 * q_head_stride
    ];

    const float q1 = q_rot[
        q_base0 + 1 * q_head_stride
    ];

    const float q2 = q_rot[
        q_base0 + 2 * q_head_stride
    ];

    const float q3 = q_rot[
        q_base0 + 3 * q_head_stride
    ];
        ...
        for (int t = 0; t < TILE_SIZE; ++t) {
            ...
            if (valid) {
                ...
                const int64_t data_base =
                    block_base
                    + static_cast<int64_t>(token_in_block)
                        * KV_HEADS
                        * DATA_BYTES_PER_SLOT
                    + static_cast<int64_t>(kvh)
                        * DATA_BYTES_PER_SLOT;
...
#pragma unroll 1
    ...
    const int64_t out_head_stride =
        static_cast<int64_t>(NUM_SPLITS)
        * (HEAD_DIM + 1);

    const int64_t out_split_stride =
        HEAD_DIM + 1;
...
torch::Tensor tq4_cuda_v1_cuda(
    ...
    TORCH_CHECK(
        kv_cache.is_cuda(),
        "kv_cache must be CUDA"
    );

    TORCH_CHECK(
        block_table.is_cuda(),
        "block_table must be CUDA"
    );

    TORCH_CHECK(
        seq_lens.is_cuda(),
        "seq_lens must be CUDA"
    );
    ...
    TORCH_CHECK(
        q_rot.scalar_type() == torch::kFloat32,
    ...
    TORCH_CHECK(
        mid_o.scalar_type() == torch::kFloat32,
        "mid_o must be float32"
    );
    ...
    TORCH_CHECK(
        mid_o.is_contiguous(),
        "mid_o must be contiguous"
    );


    TORCH_CHECK(
        q_rot.dim() == 3,
        "q_rot must be [B, Hq, D]"
    );

    TORCH_CHECK(
        q_rot.size(1) == Q_HEADS,
        "CUDA V1 requires Hq=32"
    );

    TORCH_CHECK(
        q_rot.size(2) == HEAD_DIM,
        "CUDA V1 requires D=128"
    );


    TORCH_CHECK(
        kv_cache.dim() == 4,
        "kv_cache must be [blocks,16,8,134]"
    );

    TORCH_CHECK(
        kv_cache.size(1) == BLOCK_SIZE,
        "CUDA V1 requires block_size=16"
    );

    TORCH_CHECK(
        kv_cache.size(2) == KV_HEADS,
        "CUDA V1 requires Hk=8"
    );

    TORCH_CHECK(
        kv_cache.size(3) == SLOT_SIZE,
        "CUDA V1 requires slot_size=134"
    );
    ...
    TORCH_CHECK(
        seq_lens.size(0) == B,
        "seq_lens batch mismatch"
    );

    TORCH_CHECK(
        centroids.numel() == 16,
        "4-bit path requires 16 centroids"
    );

    TORCH_CHECK(
        mid_o.size(0) == B
        && mid_o.size(1) == Q_HEADS
        && mid_o.size(2) == NUM_SPLITS
