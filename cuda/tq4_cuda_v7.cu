...
__global__ __launch_bounds__(THREADS) void tq4_cuda_v7_stage2_kernel(
    ...
    const int64_t mid_base =
        (static_cast<int64_t>(b) * Q_HEADS + qh) * head_stride;

    if (tid < NUM_SPLITS) {
        weights[tid] = mid_o[
            mid_base + static_cast<int64_t>(tid) * (HEAD_DIM + 1) + HEAD_DIM];
    }
    __syncthreads();

    if (tid < NUM_SPLITS) {
        const float split_lse = weights[lane];
        const float max_lse = __shfl_sync(
            FULL_MASK, warp_max(split_lse), 0);
        const float weight = expf(split_lse - max_lse);
        const float weight_sum = __shfl_sync(
            FULL_MASK, warp_sum(weight), 0);
        weights[lane] = weight;
        if (lane == 0) {
...
#pragma unroll
    for (int split = 0; split < NUM_SPLITS; ++split) {
        acc += weights[split] * mid_o[
            mid_base + static_cast<int64_t>(split) * (HEAD_DIM + 1) + tid];
    }
    output[(static_cast<int64_t>(b) * Q_HEADS + qh) * HEAD_DIM + tid]
        = acc * inv_sum;
    if (tid == 0) {
...
torch::Tensor tq4_cuda_v7_stage2_cuda(
    ...
    TORCH_CHECK(output.sizes() == torch::IntArrayRef(
        {B, Q_HEADS, HEAD_DIM}), "invalid output shape");
    TORCH_CHECK(lse.sizes() == torch::IntArrayRef(
        {B, Q_HEADS}), "invalid lse shape");
    c10::cuda::CUDAGuard guard(mid_o.device());
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    tq4_cuda_v7_stage2_kernel<<<dim3(B, Q_HEADS), THREADS, 0, stream>>>(
        mid_o.data_ptr<float>(), output.data_ptr<float>(),
        lse.data_ptr<float>(), B);
