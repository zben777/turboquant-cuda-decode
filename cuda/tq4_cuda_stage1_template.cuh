...
__global__ __launch_bounds__(THREADS) void TQ4_STAGE1_KERNEL(
    ...
    if (split_end - split_start != SPLIT_TOKENS || (split_start & 15)) return;
    ...
    for (int i = tid; i < GQA * HEAD_DIM; i += THREADS) {
        ...
        q_s[row][d] = __float2half(
            q_rot[(static_cast<int64_t>(b) * Q_HEADS + qh) * HEAD_DIM + d]);
    }
...
#pragma unroll
    for (int tile = 0; tile < SPLIT_TOKENS; tile += TILE) {
        ...
        for (int i = tid; i < TILE * PACKED_WORDS; i += THREADS) {
            ...
            const uint32_t packed_k4 = *reinterpret_cast<const uint32_t*>(
                cache + data_base[t] + byte);
            const uint32_t packed_v4 = *reinterpret_cast<const uint32_t*>(
                cache + data_base[t] + K_BYTES + byte);
#pragma unroll
            for (int j = 0; j < 4; ++j) {
                ...
                *reinterpret_cast<__half2*>(&k_s[t][d]) = __halves2half2(
                    __float2half(c_lo * k_norm[t]),
                    __float2half(c_hi * k_norm[t]));
                *reinterpret_cast<__half2*>(&v_s[t][d]) = __halves2half2(
                    __float2half((packed_v & 15) * v_scale[t] + v_zero[t]),
...
#pragma unroll
        ...
        running_m = new_m;
        if (lane < TILE) p_s[warp][lane] = __float2half(p);
        if (lane == 0) tile_alpha[warp] = alpha;
        __syncthreads();
...
#pragma unroll
        for (int i = 0; i < out0.num_elements; ++i) {
            if ((i & 2) == 0) {
                const int fragment_col = 2 * (lane & 3) + (i & 1)
                    + ((i & 4) ? 8 : 0);
