...
constexpr int K_BYTES = 64, DATA_BYTES = 128;
constexpr int SLOT_SIZE = 134, NUM_FIELDS = 3;
constexpr int K_NORM = 0, V_SCALE = 1, V_ZERO = 2;
constexpr int META_OFFSET = BLOCK_SIZE * KV_HEADS * DATA_BYTES;
constexpr int THREADS = 128, TILE = 16, MAX_TOKENS = 128;
    ...
    for (int tile = 0; tile < token_count; tile += TILE) {
        if (tid < TILE) {
            ...
            if (local < token_count) {
                ...
                data_base[tid] = 0;
                k_norm[tid] = v_scale[tid] = v_zero[tid] = 0.0f;
            }
        }
        __syncthreads();
        for (int i = tid; i < TILE * K_BYTES; i += THREADS) {
            ...
            if (tile + t < token_count) {
                packed_k = cache[data_base[t] + byte];
                packed_v = cache[data_base[t] + K_BYTES + byte];
            }
            const int d = byte * 2;
            k_s[t][d] = __float2half(centroids[packed_k & 15] * k_norm[t]);
            k_s[t][d + 1] = __float2half(centroids[packed_k >> 4] * k_norm[t]);
