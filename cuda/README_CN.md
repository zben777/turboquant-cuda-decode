...
`baseline/` 中的 Python 入口会把这些文件 JIT 编译成 PyTorch extension。
...
Git 忽略；在仓库根目录执行 `./baseline/run.sh clean` 即可删除。
...
```bash
./baseline/run.sh smoke       # 编译 V7 并执行短回归
./baseline/run.sh benchmark   # 编译和测量 V1-V7
./baseline/run.sh clean       # 删除生成的 build_v* 目录
```

全部测试入口见 [`../baseline/README_CN.md`](../baseline/README_CN.md)。
