...
def main():
    ...
    mimic_out, mimic_lse = (
        triton_mimic_reference_one_split(
            q_rot,
            kv_cache,
            block_table,
            seq_lens,
    ...
    if not (
        ...
        print()
        print(
            "CUDA V1 correctness check FAILED."
        )
        ...
        print(
            "Send the diff values to me first."
        )
    ...
    print(
        "CTA mapping       : "
        "1 batch x 1 KV head x 1 split"
    )

    print(
        "Q mapping         : "
        "one thread owns one D coordinate "
        "for all 4 GQA heads"
    )
