...
def tq4_decode_stage1_v2(
    ...
    for start_n in range(
        ...
        # ----------------------------------------------------
        norm_u16 = tl.load(
            KV_cache_u16_ptr + knorm_u16_addrs,
            mask=kv_mask_1d,
            other=0,
        )
        vec_norms = norm_u16.to(
            tl.float16,
            bitcast=True,
        ).to(tl.float32)
        ...
        v_idx = ((val_byte >> v_nibble_shift[None, :]) & 0xF).to(tl.float32)
        # ----------------------------------------------------
        # V metadata
        # ----------------------------------------------------
        scale_u16 = tl.load(
            KV_cache_u16_ptr + vscale_u16_addrs,
            mask=kv_mask_1d,
            other=0,
        )
        zero_u16 = tl.load(
            KV_cache_u16_ptr + vzero_u16_addrs,
            mask=kv_mask_1d,
            other=0,
        )
        v_scales = scale_u16.to(
            tl.float16,
            bitcast=True,
        ).to(tl.float32)
        v_zeros = zero_u16.to(
...
def main():
    ...
    print("Launching standalone " "TurboQuant V2 Stage1...")
    ...
    finite = torch.isfinite(mid_o).all().item()
    nan_count = torch.isnan(mid_o).sum().item()
    inf_count = torch.isinf(mid_o).sum().item()
    print(
        "finite      :",
        bool(finite),
    )
    ...
    if not finite:
        raise RuntimeError("Stage1 output contains NaN/Inf")
    print()
    print("Standalone TQ4 " "SoA Triton V2 Stage1: OK")
