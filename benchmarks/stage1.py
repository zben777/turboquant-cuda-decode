...
def parse_args():
    ...
    parser.add_argument("--include-cuda-v9", action="store_true")
...
def build_cuda_v9_extension():
    os.environ["TORCH_CUDA_ARCH_LIST"] = CUDA_ARCH_LIST
    os.environ["PATH"] = f"{Path(sys.executable).parent}:{os.environ['PATH']}"
    build_dir = CUDA_DIR / "build_v9"
    build_dir.mkdir(parents=True, exist_ok=True)
    return load(
        name="tq4_cuda_v9_ext",
        sources=[
            str(CUDA_DIR / "tq4_cuda_v9_bind.cpp"),
            str(CUDA_DIR / "tq4_cuda_v9.cu"),
        ],
        extra_cflags=["-O3"],
        extra_cuda_cflags=["-O3", "-lineinfo", "--use_fast_math"],
        build_directory=str(build_dir),
...
def main():
    ...
    mid_cuda_v9 = torch.empty_like(mid_aos)
    cuda_ext = None
    ...
    cuda_v9_ext = build_cuda_v9_extension() if args.include_cuda_v9 else None
    ...
    def run_cuda_v9():
        assert cuda_v9_ext is not None
        cuda_v9_ext.tq4_cuda_v9(
            q_rot,
            soa_cache,
            block_table,
            seq_lens,
    ...
    if args.include_cuda_v9:
        outputs.append(("CUDA V9", mid_cuda_v9))
    ...
    if args.include_cuda_v9:
        comparisons.append(("CUDA V9", mid_cuda_v9))
    ...
    if args.include_cuda_v9:
        runners.append(("CUDA V9", run_cuda_v9))
    ...
    if args.include_cuda_v9:
        v9_ratio = medians["CUDA V9"] / medians["SoA Triton V2-fixed"]
        if v9_ratio >= 1.0:
            print(f"CUDA V9 / V2-fixed     : {v9_ratio:.3f}x slower")
        else:
            print(f"CUDA V9 vs V2-fixed    : {1.0 / v9_ratio:.3f}x faster")
        if args.include_cuda_v8:
            print(f"CUDA V9 vs CUDA V8     : {medians['CUDA V8'] / medians['CUDA V9']:.3f}x")
